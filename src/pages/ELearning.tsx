...
const ELearning = () => {
  ...
  const checkUser = async () => {
    try {
      ...
      // If trying to access create page without being logged in
      if (location.pathname.includes("create") && !user) {
        navigate("/login");
        toast({
          title: "Authentication Required",
          description: "Please login to create courses",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleTabChange = (
    tab: "browse" | "create" | "my-courses" | "enrolled"
  ) => {
    if (tab === "create" && !user) {
      navigate("/e-learning");
      // navigate("/login");
            ...
            <div className="flex items-center space-x-2 md:space-x-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem onClick={() => handleTabChange("browse")}>
                    Browse Courses
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleTabChange("enrolled")}>
                    My Enrolled Courses
                  </DropdownMenuItem>
                  <DropdownMenuItem
