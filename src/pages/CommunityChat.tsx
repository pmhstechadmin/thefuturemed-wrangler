...
// //       .from('community-files') // <== your Supabase bucket
...
// //     const { data: publicUrlData } = supabase.storage.from('community-files').getPublicUrl(filePath);
...
// //       if (insertError) {
// //         toast({ title: 'Error', description: 'Could not send file message.', variant: 'destructive' });
// //       }
// //     }
// //   };
...
// // //********************************* */
// // const sendMessage = async () => {
// //   if ((!newMessage.trim() && !selectedFile) || !user || !communityId || sending) return;
...
// //       const filePath = `${communityId}/${Date.now()}-${selectedFile.name}`;
...
// //                     className={`flex items-center gap-2 ${(newMessage.trim() || selectedFile) ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-200 text-gray-500'
// //                       }`}
// //                   >
// //                     <Send className="h-4 w-4" />
// //                     Send
...
// //             if (file) {
// //               setSelectedFile(file); // Just store it
// //             }
// //           }}
// //         />
...
// //         fileInputRef.current.capture = 'environment'; // opens camera on mobile
// //       } else if (type === 'image') {
// //         fileInputRef.current.accept = 'image/*';
// //         fileInputRef.current.removeAttribute('capture'); // remove capture if not needed
// //       } else {
...
// //           ref={fileInputRef}
// //           className="hidden"
// //           accept="*/*" // this will be dynamically updated
// //           onChange={(e) => {
// //             const file = e.target.files?.[0];
// //             if (file) {
// //               setSelectedFile(file); // stores the file (image or other)
// //             }
// //           }}
// //         />
...
//       if (selectedFile) {
//         const fileExt = selectedFile.name.split('.').pop();
//       const sanitizedFileName = selectedFile.name.replace(/\s+/g, '_').replace(/[^\w.-]/gi, '');
...
//               e.target.value = "";// stores the file (image or other)
//             }
//           }}
//         />
