...
// const HostSeminar = (updateseminarData) => {
//   const [hostName, setHostName] = useState("");
//   const [topic, setTopic] = useState("");
//   const [description, setDescription] = useState("");
//   const [date, setDate] = useState<Date>();
...
//         if (!isNaN(numValue)) {
//           updateseminarData({ [field]: numValue >= 0 ? numValue : 0 });
//         }
//       } else {
...
//       });
//     } else {
//       updateseminarData({ is_paid: false, price: undefined });
//     }
//   };
...
//                   // onChange={(value) => setDescription({ description: value })}
//                   modules={modules}
//                   formats={formats}
//                 />
//               </div>
...
//                   Please note that the default value of zero in the input field
//                   will be automatically replaced as you start typing a new
//                   number
//                 </p>
//               </div>
//             )}
...
const [certificate, setCertificate] = useState<boolean>(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate price if it's a paid seminar
    if (isPaid && (!price || price <= 0)) {
      toast({
        title: "Error",
        description: "Please enter a valid price for the paid seminar.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // Insert seminar with is_paid and price fields
      const { data: seminarData, error: seminarError } = await supabase
        .from("seminars")
        .insert({
          host_id: session.user.id,
          ...
          host_country: country,
          type: seminarType,
          is_certificate: certificate, // Default to false; can be updated later
          is_paid: isPaid,
          currency: currency,
          price: isPaid ? price : 0, // Store price only if it's a paid seminar
        })
        .select()
        .single();
...
`,
  ...
  const handleCertificateToggle = (checked: boolean) => {
    setCertificate(checked); // directly stores true/false
  };
  ...
  required
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent className="max-h-60 overflow-y-auto">
                      <SelectItem value="webminar">E-Seminar</SelectItem>
                      <SelectItem value="conference">E-Conference</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                <div className="flex flex-col justify-center">
