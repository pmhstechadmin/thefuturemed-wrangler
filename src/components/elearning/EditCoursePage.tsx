          ...
          modules.map(async (module) => {
            ...
            return {
              ...
              content: (content?.map((c) => ({
                ...
                title: c.content_title,
                content: c.content_text,
                file_url: c.content_url,
                file_name: c.content_title,
              })) || []) as ContentData[],
              mcq_questions: (questions?.map((q) => ({
                question_text: q.question_text,
                option_a: q.option_a,
      ...
      for (let i = 0; i < data.modules.length; i++) {
        ...
        for (const content of module.content) {
          ...
          if (content.type === "text") {
            contentData.content_text = content.content;
          } else {
            console.log("calling insert");
            contentData.file_url = content.file_url;
            contentData.file_name = content.file_name;
            contentData.url = content.file_url;
            contentData.file_size = content.file_size;
          }
