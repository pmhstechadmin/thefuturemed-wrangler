...
// Resize images inside HTML content
const resizeImagesInHtml = (html: string): string => {
  return html.replace(/<img([^>]*)>/g, (match, group1) => {
    if (/style\s*=/.test(group1)) {
      return `<img${group1.replace(
...
async function getSignedUrlIfNeeded(
  ...
  try {
    ...
    if (url.pathname.includes("/course-content/")) {
    ...
    //   const { data, error } = await supabase.storage
    //     .from("course-content")
    //     .createSignedUrl("course-content/1755776541750-module-1.pdf", 60000);
...
//         .from("course-content")
//         .createSignedUrl(
//           "course-content/course-content/1755776541750-module-1.pdf",
//           60 * 60
//         );
...
// Component to display content blocks
function ContentBlock({ item }: { item: ContentData }) {
  ...
  useEffect(() => {
    const fetchSignedUrl = async () => {
      if (item.content_type === "pdf" || item.content_type === "video") {
        setLoading(true);
        const url = await getSignedUrlIfNeeded(item.content_url);
        console.log("item url",item.content_url);
        setSignedUrl(url);
        setLoading(false);
      }
    };

    console.log("dcdddcd", item.content_type);
  ...
  return (
    <Card className="border bg-white/90">
      <CardHeader className="flex items-center gap-2 py-3 px-4">
        ...
        <h5 className="font-medium text-gray-800">{item.content_title}</h5>
      </CardHeader>
      ...
      <CardContent className="px-4 pb-4">
        {item.content_type === "text" && item.content_text && (
          <div
            className="prose max-w-none text-gray-800 text-justify"
            dangerouslySetInnerHTML={{
              __html: resizeImagesInHtml(item.content_text),
            }}
          />
        )}
...
// Main component for Modules Accordion
export default function ModulesAccordion({
  modules,
  activeModuleId,
  setActiveModuleId,
    ...
    <div className="space-y-6">
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold">{module.title}</h3>
                    {module.description && (
                      <div className="text-sm text-gray-700 text-justify mt-1">
                        <div
                          className="prose max-w-none text-gray-800"
                          dangerouslySetInnerHTML={{
                            __html: resizeImagesInHtml(module.description),
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle
              className={dialog.correct ? "text-green-600" : "text-red-600"}
            >
              {dialog.correct ? "Correct ✅" : "Incorrect ❌"}
            </DialogTitle>
            <DialogDescription>{dialog.message}</DialogDescription>
