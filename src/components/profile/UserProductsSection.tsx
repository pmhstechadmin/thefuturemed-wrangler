...
//   const totalProducts =
//     products.seminars.length +
//     products.courses.length +
//     products.publications.length +
//     products.jobPostings.length;
...
//                             <Button
//                               onClick={() =>
//                                 handleNavigateToProduct("seminars")
//                               }
//                               size="sm"
//                               variant="ghost"
//                               className="h-6 px-2"
...
export const UserProductsSection = ({ userId }: UserProductsSectionProps) => {
  ...
  const fetchUserProducts = async () => {
    try {
      ...
      const [
        { data: seminars, error: seminarsError },
        { data: courses, error: coursesError },
        { data: jobProviders, error: jobProvidersError },
        { data: blogs, error: blogsError },
      ] = await Promise.all([
        supabase.from("seminars").select("*").eq("host_id", userId),
        supabase.from("courses").select("*").eq("creator_id", userId),
        supabase.from("job_providers").select("*").eq("user_id", userId),
        supabase.from("blog").select("*").eq("user_id", userId),
      ]);
      ...
      setProducts({
        seminars: seminars || [],
        courses: courses || [],
        publications: [], // Still empty as per original
        jobPostings: jobProviders || [],
        blogs: blogs || [],
      });
    } catch (error) {
              ...
              <>
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-semibold flex items-center gap-2">
                      <Briefcase className="h-5 w-5 text-orange-600" />
                      My Jobs ({products.jobPostings.length})
                    </h3>
                    <Button
                      onClick={() => handleNavigateToProduct("myjob")}
                      variant="outline"
              ...
              <>
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-semibold flex items-center gap-2">
                      <FileText className="h-5 w-5 text-teal-600" />
                      My Blogs ({products.blogs.length})
                    </h3>
                    <Button
                      onClick={() => handleNavigateToProduct("blogs")}
                      variant="outline"
